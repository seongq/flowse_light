...
class NCSNpp(nn.Module):
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        num_channels = 6  # x.real, x.imag, y.real, y.imag, c.real, c.imag
    ...
    def forward(self, x, time_cond):
        ...
        x_chans.append(torch.cat([ 
            torch.cat([x[:,[in_chan],:,:].real, x[:,[in_chan],:,:].imag ], dim=1) for in_chan in range(self.input_channels // 2)],
                dim=1)
            )
        x = torch.cat(x_chans, dim=1) #6*D
        # print(x.size())
